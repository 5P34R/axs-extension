...
const axs_functions_1 = require("./axs-functions");
const form_functions_1 = require("./form-functions");
/**
 * Helper function to create a completion item from our simple function format
 */
function createCompletionItem(func) {
...
function provideCompletions(document, position) {
    ...
    if (linePrefix.endsWith('ax.')) {
        ...
        Object.values(axs_functions_1.AXS_FUNCTIONS).forEach(category => {
    ...
    // Show form functions when typing "form."
        ...
        form_functions_1.FORM_FUNCTIONS.forEach(func => {
            completions.push(createCompletionItem(func));
        });
    }
    // Show menu functions when typing "menu."  
    else if (linePrefix.endsWith('menu.')) {
        form_functions_1.MENU_FUNCTIONS.forEach(func => {
            completions.push(createCompletionItem(func));
        });
    }
    // Show AxCommand functions when typing on command objects (like "cmd." or "_cmd_something.")
        ...
        form_functions_1.AXCOMMAND_FUNCTIONS.forEach(func => {
            completions.push(createCompletionItem(func));
        });
    }
